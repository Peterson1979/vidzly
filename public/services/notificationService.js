...
import { NOTIFICATION_PREFERENCES_KEY, LAST_ACTIVE_STORAGE_KEY, STREAK_STORAGE_KEY, APP_NAME, INACTIVITY_THRESHOLD_DAYS, LAST_NOTIFICATION_TIME_KEY, NOTIFICATION_COOLDOWN_MS, LAST_WEEKLY_NOTIFICATION_TIME_KEY, LAST_HUMOROUS_NOTIFICATION_TIME_KEY, HUMOROUS_NOTIFICATION_INTERVAL_MS, DEFAULT_LOGO_SVG_STRING, IN_APP_NOTIFICATIONS_STORAGE_KEY, MAX_IN_APP_NOTIFICATIONS, NEW_IN_APP_NOTIFICATION_EVENT, DEFAULT_NOTIFICATION_PREFERENCES } from '../constants';
...
const displayBrowserNotification = (title, options) => {
    if (typeof Notification === 'undefined' || Notification.permission !== 'granted')
        return;
...
export const addInAppNotificationToStore = (title, body, type, link, videoId) => {
    try {
    ...
    if (Notification.permission === 'default') {
        try {
            ...
            if (permission === 'granted') {
                ...
                addInAppNotificationToStore("Browser Notifications Enabled", "You've allowed browser notifications. You can manage these and other alerts in settings.", InAppNotificationType.GENERIC);
            }
            else {
                console.log('Browser Notification permission denied.');
                addInAppNotificationToStore("Browser Notifications Not Enabled", "You can enable browser notifications via your browser settings if you change your mind.", InAppNotificationType.GENERIC);
            }
        }
        catch (error) {
...
export const updateStreakAndNotify = (allVideos) => {
    const prefs = getNotificationPreferences();
    ...
    let streak = rawStreakData ? JSON.parse(rawStreakData) : { count: 0, lastUpdated: 0 };
...
export const checkAndSendScheduledNotifications = (allVideos) => {
    const prefs = getNotificationPreferences();
    // Daily Hot Content
...
export const checkAndSendInactivityNotification = (allVideos) => {
    const prefs = getNotificationPreferences();
    if (!prefs.inactivityReengagement)
        return;
    const lastActive = parseInt(localStorage.getItem(LAST_ACTIVE_STORAGE_KEY) || '0');
...
export const checkAndSendHumorousNotification = (allVideos) => {
    const prefs = getNotificationPreferences();
    if (!prefs.humorousSurprising)
        return;
...
export const announceNewFeature = (featureTitle, featureBody, link) => {
    addInAppNotificationToStore(`✨ New Feature: ${featureTitle}`, featureBody, InAppNotificationType.FEATURE_ANNOUNCEMENT, link || '/settings' // Default link to settings or a specific feature page
    );
