...
const generateMockRecommendations = (favoriteItems, allVideos, count = 5) => {
...
export const getPersonalizedFeed = async (favoriteItems, allVideos) => {
    ...
    const promptFavoriteVideos = favoriteVideoDetails.slice(0, 10).map(v => ({
        id: v.id,
        title: v.title,
        description: v.description.substring(0, 100) + "..."
    }));
    const promptAllVideos = allVideos.slice(0, 50).map(v => ({
        id: v.id,
        title: v.title,
...
`;
        ...
        const finalRecommendations = parsedData.recommendedVideoIds
            ...
            .slice(0, desiredRecommendationCount);
