...
// --- API CALL WRAPPER ---
async function fetchYouTubeData(endpoint, params, apiKey) {
...
const useMockDataForYoutube = (reason) => {
...
export const fetchVideos = async ({ searchTerm = '', pageToken, count, }) => {
    if (!YOUTUBE_API_KEY || YOUTUBE_API_KEY === PLACEHOLDER_API_KEY_NOT_SET) {
        ...
        if (searchTerm.trim()) {
            ...
            sourceVideos = allMockVideos.filter(v => v.title.toLowerCase().includes(lowerSearchTerm) ||
    ...
    try {
        ...
        if (searchTerm.trim()) {
            ...
            if (pageToken)
                popularParams.pageToken = pageToken;
        ...
        const videos = videoDataItems.map((item) => ({
