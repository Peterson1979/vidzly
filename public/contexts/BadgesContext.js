...
import { BadgeId, InAppNotificationType } from '../types';
...
export const BadgesProvider = ({ children }) => {
    ...
    const checkAndAwardBadges = useCallback((currentFavoritesForCheck = []) => {
        setEarnedBadges(prevEarned => {
            let newBadgesAwarded = false;
            const updatedEarnedBadges = [...prevEarned];
            ALL_BADGES.forEach(badge => {
                if (updatedEarnedBadges.includes(badge.id))
                    return;
                let criteriaMet = false;
                switch (badge.id) {
                    case BadgeId.WATCH_10:
                        if (progress.watchedVideosCount >= badge.criteriaCount)
                            criteriaMet = true;
                        break;
                    case BadgeId.AI_INSIGHTS_3:
                        if (progress.aiInsightsUsedCount >= badge.criteriaCount)
                            criteriaMet = true;
                        break;
                    case BadgeId.FAVORITE_5_DISTINCT_CATEGORIES:
                        const distinctCategories = new Set(currentFavoritesForCheck
                            .map(fav => fav.categoryId)
                            .filter(catId => catId !== null && catId !== undefined));
                        if (distinctCategories.size >= badge.criteriaCount)
                            criteriaMet = true;
                        break;
                }
                if (criteriaMet) {
                    ...
                    addInAppNotificationToStore(`Badge Unlocked: ${badge.name}!`, badge.description, InAppNotificationType.BADGE_EARNED, '/achievements' // Link to achievements view
                    );
    ...
    const checkFavoriteBadges = useCallback((currentFavorites) => {
    ...
    return (_jsx(BadgesContext.Provider, { value: contextValue, children: children }));
};
